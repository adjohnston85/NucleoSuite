"""Probabilistic nucleosome scoring with length-adaptive sinusoidal kernels."""
...
def scoring_track_names(scoring_method: str = "pns") -> tuple[str, str, str]:
    """Return the PNS smoothed, signed and non-negative reference track names."""
...
def sinusoidal_nucleosome_kernel(total_length: int) -> np.ndarray:
    ...
    absolute mass 200, independent of support width.
    """
    if total_length < 3:
    ...
    # Remove roundoff asymmetry before the signed masses are measured. This
    # preserves the sampled cosine geometry even near a zero crossing.
    raw = 0.5 * (raw + raw[::-1])
...
def precompute_distributions(
    ...
    for fragment_length in fragment_lengths:
        ...
        # The non-negative reference is a vertical translation of the complete
        # signed wave. It is not a separately normalized probability kernel.

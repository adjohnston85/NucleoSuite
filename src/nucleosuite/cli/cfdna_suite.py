"""Launch and parallelize the packaged cfDNA full-suite workflow."""
...
def _resolve_contigs(argv: Sequence[str]) -> tuple[list[str], list[tuple[str, int]], list[tuple[str, int]]]:
    ...
    try:
        ...
        raise RuntimeError("cfdna-suite multicontig processing requires pysam") from exc
    ...
    if any(":" in spec for spec in specs):
        raise ValueError("cfdna-suite multicore mode accepts whole contigs, not coordinate ranges")
    selected: list[str] = []
    for chrom in specs:
...
def _parallel_main(
    ...
    with ProcessPoolExecutor(max_workers=min(cores, len(active_contigs))) as executor:
        ...
        for contig in active_contigs:
            ...
            future = executor.submit(
                ...
                str(
                    contig_out
                    / (
                        f"{worker_sample}_cfdna-suite.log"
                        if randomized_mode
                        else "cfdna-suite.log"
                    )
                ),
                worker_environment,
            )
        ...
        for future in as_completed(futures):
            ...
            if code:
                ...
                print(f"Completed cfDNA suite for {contig}")
    if failures:
        print(
            "WARNING: Some per-contig cfDNA workflows recorded failures. "
    ...
    combined_log = combined_root / (
        f"{suite_sample}_cfdna-suite-combined.log"
        if randomized_mode
        else "cfdna-suite-combined.log"
    )
    code = _run_shell(
        script_path,
        final_args,
    ...
    if failures:
        raise RuntimeError(
            "The cfDNA suite attempted all remaining steps but recorded failures: "
            + "; ".join(failures)
        )
    return 0
...
def validate_argv(argv: Sequence[str] | None = None) -> None:
    ...
    script_resource = files("nucleosuite").joinpath("resources/cfdna_full_suite.sh")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if paired:
        ...
        # FDR annotation is a BED extension even when bigBed output is also wanted.

...
def test_wps_peak_caller_is_default():
    ...
    args = build_parser().parse_args(["wps", "-b", "sample.bam"])
...
def test_call_peaks_smoothing_defaults():
...
def test_pns_cli_defaults_and_native_bigbed_scale():
    from nucleosuite.cli.main import build_parser
    ...
    assert args.score_tracks == ["pns", "posPNS"]
...
def test_pns_auto_mode_resolves_default_fragment_bounds_from_mode(monkeypatch):
    from nucleosuite.cli.main import build_parser
    ...
    monkeypatch.setattr(pns_cli, "resolve_fragment_mode", fake_resolve)
    mode, _estimate, source, _seed = pns_cli._resolve_mode_and_fragment_range(args)
    assert mode == 165
    assert source == "automatic"
    assert captured == {"estimate_range": (137, 197), "value": "auto", "command": "pns"}
    assert (args.frag_lower, args.frag_upper) == (135, 195)


def test_pns_explicit_mode_uses_mode_padding_when_bounds_are_omitted():
    from nucleosuite.cli.main import build_parser
    from nucleosuite.cli import pns as pns_cli

    args = build_parser().parse_args(["pns", "--bam", "sample.bam", "--mode", "152"])
    pns_cli._resolve_mode_and_fragment_range(args)
    assert (args.frag_lower, args.frag_upper) == (122, 182)


def test_pns_run_uses_only_pns_tracks_and_native_bigbed_scale(monkeypatch, tmp_path):
    from nucleosuite.cli.main import build_parser
    from nucleosuite.cli import pns as pns_cli
    import nucleosuite.parallel as parallel

    args = build_parser().parse_args([
        "pns", "--bam", "sample.bam", "--mode", "167",
    ...
    assert pns_cli.run(args) == 0
    assert captured == {
        "command": "pns", "tracks": ["pns", "posPNS"], "scale": 1.0,
    }


def test_pns_command_rejects_removed_scoring_option():
    from nucleosuite.cli.main import build_parser
    ...
    assert "pns" in choices
    assert "pns_smoothed" not in choices
...
def test_tracks_uses_pns_and_native_bigbed_default(monkeypatch):
    from nucleosuite.cli.main import build_parser
    ...
    assert tracks_cli.run(args) == 0
    assert captured == [("pns", 1.0)]


def test_bigbed_score_scale_is_configurable():
    from nucleosuite.cli.main import build_parser

    parser = build_parser()
    pns = parser.parse_args(["pns", "--bam", "sample.bam", "--bigbed-score-scale", "250"])
...
def test_pns_bed_scores_keep_six_decimal_places(tmp_path):
    ...
    write_records(
        str(path),
        [{"chrom": "chr1", "region_start": 10, "region_end": 20,
          "region_centre": 15, "peak_score": 3.14159265}],
        "nuc", 1.0, "w",
    )

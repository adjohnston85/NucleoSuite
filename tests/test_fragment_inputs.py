...
def test_fragments_command_combines_interval_inputs(tmp_path: Path):
    ...
    assert (tmp_path / "combined.fragments.bed").read_text().splitlines() == [
        "chr1\t10\t20",
        "chr1\t30\t45",
    ]
    assert (tmp_path / "combined.fragment_length_distribution.png").exists()
...
def test_fragments_command_streams_multiple_contigs_to_combined_bed(tmp_path: Path):
    ...
    assert (tmp_path / "out.fragments.bed").read_text().splitlines() == [
        "chr1\t10\t20",
        "chr2\t5\t18",
    ]

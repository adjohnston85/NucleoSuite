...
Automatic output stems contain at most three central analysis-parameter tokens so long input names do not exceed filesystem filename limits. A supplied `--output-prefix` is a base prefix and receives the same shortened suffix. Every generated plot has a metadata TSV sidecar containing the complete command parameter set, including settings that are not present in the filename.

...
The cfDNA and MNase suites first mean-scale their combined PNS-derived nucleosome-region and breakpoint-peak BED scores to 100, then pass those normalized BEDs to `peak-score-frequency` with `--score-scale 1`. No additional suite-level histogram scaling is applied.

...
Default `chip-suite` Stage 1 also preserves the treatment candidate coordinates and other BED fields, replaces column 5 with maximum condition-mean scaled treatment coverage, and appends BH FDR. A dot indicates that replicate-based inference was unavailable. The accompanying `target_peak_replicate_statistics.tsv` contains every treatment/control replicate maximum, group means, conservative excess, conservative fold and log2 enrichment with pseudocount 1, the all-controls gate, Stage 2 selection flag, p-value and FDR. The gate is the default individual-peak selector; optional p-value or FDR cutoffs may narrow it. Cluster seeds pass the gate and p < 0.05 by default; neighbouring gated peaks can extend the cluster without passing the seed p-value.

`target_clusters_seeded.tsv` reports cluster ID, chromosome, boundaries, seed count, total gated-member count, successfully bridged non-gated count, cluster score, maximum member peak score, strongest-member discovery summit, minimum seed p-value, and maximum seed FDR. Cluster BED coordinates span the outermost gated members and append the maximum member scaled-coverage score after the seed FDR. A bridged non-gated candidate can lie inside that span but is not counted as a member and contributes no score.
...
`mean-scale --reference-bigwig` divides one BigWig by the finite, non-zero mean of another. `chip-suite` uses this relationship to normalize PNS by `posPNS`, BNS by `posBNS`, or TNS by `posTNS` separately for treatment and control peak discovery. Discovery tracks use the resolved mode ±30 bp by default. A separate pass retains raw 1–1,000 bp coverage and writes coverage BigWigs divided by their own finite non-zero mean and multiplied by 100. Maximum values from those broad-range scaled coverage tracks become the Stage 1 peak scores. For aggregate positioning, treatment PNS is independently divided by each replicate's mean `posPNS` before the replicate tracks are averaged.
...
`chip_stage1_manifest.json` records the score method, treatment/control modes, fragment limits, contigs, input mode, Stage 1 statistical method, peak and cluster paths, condition-mean tracks, separate treatment/control replicate records, replicate PNS/`posPNS` normalization, cluster anchors, and aggregate outputs. `chip-compare` reads these saved paths directly and validates compatible scoring geometry before measurement.

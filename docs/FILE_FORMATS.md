...
active_genes	9_Txn_Transition | 10_Txn_Elongation	1_Active_Promoter		12_Repressed	repressed_genes
weak_genes	9_Txn_Transition | 10_Txn_Elongation | 11_Weak_Txn	2_Weak_Promoter		12_Repressed	active_genes,repressed_genes
...
`empirical-peak-fdr` and paired cfDNA/MNase suite execution preserve every original observed peak field and append two final fields:

```text
chrom  start  end  name  score  strand  thickStart  thickEnd  empirical_p_value  empirical_fdr
```

`empirical_p_value` is the pooled randomized upper-tail p-value for that observed score. `empirical_fdr` is the monotonic empirical q-value obtained across score thresholds. Existing fields are preserved in their original order. These are extended BED tables rather than strict BED9 because columns after the standard BED fields carry analysis statistics. An optional `--fdr` threshold writes an additional filtered BED while retaining the complete annotated output.

Default `cutn-suite` Stage 1 preserves every treatment candidate interval and writes `target_peaks_replicate_statistics.bed`. Column 5 contains the condition-mean Stage 1 coverage measurement, and one final field contains the raw one-sided Welch p-value when the replicate-aware seed rule uses p-values; `.` marks candidates for which that statistic is not part of the selected rule. The corresponding TSV records every replicate measurement, group means, gate results, seed/member status and raw p-value. Mean scaled coverage across the complete candidate interval is the default Stage 1 replicate statistic; `--stage1-coverage-statistic max` selects the interval maximum.

`target_clusters_seeded.tsv` reports cluster ID, chromosome, boundaries, seed count, included-member count, bridged non-member count, cluster score, maximum member peak score, strongest-member discovery summit and minimum seed p-value when applicable. Cluster BED coordinates span the outermost included members.
...
`mean-scale --reference-bigwig` can explicitly divide a BigWig by the finite, non-zero mean of another. Standard PNS workflows retain native PNS and `posPNS` values. `cutn-suite` generates mode-centred PNS and broad 1–1,000 bp coverage together in one `tracks` pass per replicate. Coverage is divided by its own finite non-zero mean and multiplied by 100 for Stage 1 interval measurements; native PNS is reused for cluster-centred positioning.
...
Fragment and region commands split work by contig when the primary input has a random-access index. Supported forms are bigBed and bgzip-compressed BED/TSV with a `.tbi` or `.csi` index. Tabix-indexed fragment files also use `--chrom-sizes` or `--fasta` to supply chromosome lengths. Plain BED, ordinary gzip, and unindexed TSV inputs run serially.

...
Default `chip-suite` Stage 1 preserves treatment candidate coordinates and other BED fields, replaces column 5 with maximum condition-mean scaled treatment coverage, and appends **raw Welch p-value followed by BH FDR**. A dot indicates unavailable replicate-based inference. The complete annotated BED is `target_peaks_replicate_statistics_gate_<mode>.bed`. `target_seed_peaks_gate_<mode>_seed_p<threshold>.bed` contains exactly the `S` seed peaks that pass the selected gate and satisfy the configured seed p-value threshold, with the same appended p-value and FDR columns. `target_peak_replicate_statistics_gate_<mode>.tsv` contains every treatment/control replicate maximum, group means, mean treatment-minus-control difference, conservative `min(T)-max(C)` excess and fold/log2 enrichment, both mean and all-controls gate results, the selected gate mode and excess, Stage 2 selection flag, p-value, and FDR. The all-controls gate is the default individual-peak selector; optional p-value or FDR cutoffs may narrow it.

`target_clusters_seeded.tsv` reports cluster ID, chromosome, boundaries, seed count, included-member count, successfully bridged non-member count, cluster score, maximum member peak score, strongest-member discovery summit, minimum seed p-value, and maximum seed FDR. Cluster BED coordinates span the outermost included members. With the default all-controls gate, cluster score is the sum of minimum-treatment-minus-maximum-control excess; mean mode instead sums mean treatment-minus-control. Bridging candidates are not counted as members and contribute no score.
...
Standalone `pns` and `wps` write `*_fragment_mode_estimation.tsv`. The table records whether the mode was automatic or explicit, the resolved numeric mode, seed, bootstrap interval, sampled and in-range fragment counts, search bounds, histogram-smoothing setting, convergence result, checkpoint count, and the length:count histogram. Automatic mode estimation uses unsmoothed counts by default.

`chip_stage1_manifest.json` records the score method, treatment/control modes, fragment limits, contigs, input mode, Stage 1 statistical method, peak and cluster paths, condition-mean tracks, separate treatment/control replicate records, replicate method-matched score/positive-score normalization, cluster anchors, and aggregate outputs. `chip-compare` reads these saved paths directly and validates compatible scoring geometry before measurement.

`differential_clusters.tsv` contains cluster-locus coordinates, Stage 1 support flags, `region_origin`, source-cluster counts and IDs, every positive scaled-coverage area from the four independent replicate groups, raw and log2 group means, raw and log2 interaction differences, within-condition enrichment bounds, effect direction, replicate-consistency class, ordinary and empirical-Bayes moderated p-values, differential FDR, ordinary and moderated standard errors and 95% confidence intervals, residual and posterior variances, and status. `region_origin` is `overlap_union`, `condition1_only`, or `condition2_only`. Directional BEDs append the same field after differential FDR. P-values and FDR are blank unless every group contains at least two biological replicates.

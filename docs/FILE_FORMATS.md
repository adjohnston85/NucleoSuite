...
Default `chip-suite` Stage 1 preserves treatment candidate coordinates and other BED fields, replaces column 5 with maximum condition-mean scaled treatment coverage, and appends BH FDR. A dot indicates unavailable replicate-based inference. `target_peak_replicate_statistics.tsv` contains every treatment/control replicate maximum, group means, mean treatment-minus-control difference, conservative `min(T)-max(C)` excess and fold/log2 enrichment, both mean and all-controls gate results, the selected gate mode and excess, Stage 2 selection flag, p-value, and FDR. The mean gate is the default individual-peak selector; optional p-value or FDR cutoffs may narrow it.

`target_clusters_seeded.tsv` reports cluster ID, chromosome, boundaries, seed count, included-member count, successfully bridged non-member count, cluster score, maximum member peak score, strongest-member discovery summit, minimum seed p-value, and maximum seed FDR. Cluster BED coordinates span the outermost included members. With the default mean gate, cluster score is the sum of mean treatment-minus-control excess; all-controls mode sums minimum treatment-minus-maximum control. Bridging candidates are not counted as members and contribute no score.
